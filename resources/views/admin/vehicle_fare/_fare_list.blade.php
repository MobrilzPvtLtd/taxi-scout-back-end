<style>
    ...
    */ .tab-content {
        margin-top: 35px;
        /* Add margin to create space between tabs and table */
    }

    .nav-tabs {
        margin-top: 35px;
        /* Add margin to create space above the navigation tabs */
    }
</style>
...
<ul class="nav nav-tabs" id="zoneTabs" role="tablist">
    ...
    @foreach ($zoneNames as $zoneName)
        <li class="nav-item" role="presentation">
            <a class="nav-link custom-nav-link @if ($zoneIndex === 0) active @endif"
                id="zone-tab-{{ $zoneIndex }}" data-toggle="tab" href="#zone-{{ $zoneIndex }}" role="tab"
                aria-controls="zone-{{ $zoneIndex }}" aria-selected="true">{{ $zoneName }}</a>
        </li>
        @php
            $zoneIndex++;
        @endphp
...
<div class="tab-content" id="zoneTabsContent">
    ...
    @endif
    @foreach ($zoneNames as $zoneName)
        <div class="tab-pane fade @if ($zoneIndex === 0) show active @endif" id="zone-{{ $zoneIndex }}"
            role="tabpanel">
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <button type="button" class="btn btn-info btn-sm dropdown-toggle"
                                        ...
                                        aria-expanded="false">@lang('view_pages.action')</button>
                                    <div class="dropdown-menu w-48">
                                        <a class="dropdown-item" href="{{ url('vehicle_fare/edit', $result->id) }}">
                                            <i class="fa fa-pencil"></i>@lang('view_pages.edit')
                                        </a>
                                        <a class="dropdown-item"
                                            href="{{ url('vehicle_fare/rental_package/index', $result->zoneType->id) }}">
                                        ...
                                        @if ($result->active == 1 && $result->zoneType->zone->default_vehicle_type != $result->zoneType->vehicleType->id)
                                            <a class="dropdown-item"
                                                href="{{ url('vehicle_fare/set/default', $result->id) }}"><i
                                                    class="fa fa-dot-circle-o"></i>@lang('view_pages.set_as_default')</a>
                                        @endif
                                        @if ($result->zoneType->active)
                                            <a class="dropdown-item"
                                                href="{{ url('vehicle_fare/toggle_status', $result->id) }}">
                                        ...
                                        <a class="dropdown-item sweet-delete" href="#"
                                            data-url="{{ url('vehicle_fare/delete', $result->id) }}">
